...
namespace apus {
    ...
    void DataType::Insert(std::string& name, DataType* elem) {
        DataTypePtr elem_ptr(elem); // pointer to shared_ptr
        map_[name] = elem_ptr;
    }

    void DataType::Insert(std::string& name, DataTypePtr elem) {
        map_[name] = elem;
    }
    ...
    DataTypePtr DataType::Find(const std::string& name) {
        ...
        DispErr("There's no data type with name " + 
                name + " in DataType::Find()");
        return nullptr;
    }
    ...
    void DataTypeTable::Insert(std::string& name, DataType* elem) {
        DataTypePtr elem_ptr(elem);
        map_[name] = elem_ptr;
    }

    void DataTypeTable::Insert(std::string& name, DataTypePtr elem) {
        map_[name] = elem;
    }
